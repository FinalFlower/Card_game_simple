...
EXCLUDED_ITEMS = {'.git', '__pycache__', 'venv', 'env', '.venv', '.vscode', '.idea', '旧代码','.DS_Store','others', 'node_modules', 'dist', 'build'}
...
def get_code_contents(root_dir, script_name):
    """
    获取项目中所有 .py, .js, .css, .html 文件和 README.md 文件的内容。
    ...
    # 2. 递归查找所有 .py, .js, .css, .html 文件和其他 README.md (如果在子目录)
    ...
    for relative_path_display, absolute_path in files_to_extract_info:
        try:
            ...
            if relative_path_display.endswith(".py"):
                lang_type = "python"
            elif relative_path_display.endswith(".js"):
                lang_type = "javascript"
            elif relative_path_display.endswith(".css"):
                lang_type = "css"
            elif relative_path_display.endswith(".html"):
                lang_type = "html"
...
def main():
    ...
    # 2. 获取代码内容
    print("正在提取 .py, .js, .css, .html 和 README.md 文件内容...")
    try:
    ...
    try:
        with open(output_md_path, 'w', encoding='utf-8') as f:
            ...
            if code_contents_str:
                ...
                f.write("未找到相关代码或 README.md 文件，或提取失败。\n")
