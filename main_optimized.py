...
class CardBattleGame:
    ...
    def _display_available_characters(self):
        """显示可选角色信息"""
        print("\n可选角色：")
        characters = game_data.get_all_characters()
        for i, (char_id, char_info) in enumerate(characters.items(), 1):
            print(f"{i}. {char_info['display_name']} - {char_info['description']}")
    ...
    def _player_select_characters(self, player: Player):
        ...
        for i in range(2):  # 选择2个角色
            while True:
                print(f"\n选择第{i+1}个角色：")
                characters = game_data.get_all_characters()
                for j, char_id in enumerate(available_chars, 1):
                    char_info = characters[char_id]
                    print(f"{j}. {char_info['display_name']}")
                
                try:
                    choice = int(input("请输入选择 (数字): ")) - 1
                    if 0 <= choice < len(available_chars):
                        selected_id = available_chars.pop(choice)
                        character = self.engine.create_character(selected_id)
                        player.add_character(character)
                        print(f"已选择: {character.display_name}")
                        break
                    else:
                        print("无效选择，请重新输入！")
                except ValueError:
    ...
    def _ai_select_characters(self, player: Player):
        ...
        for i in range(2):
            selected_id = available_chars.pop()
            character = self.engine.create_character(selected_id)
            player.add_character(character)
            print(f"AI选择了: {character.display_name}")
        ...
        for i, player in enumerate(self.game_state.players, 1):
            ...
            if player.team_effects:
                ...
                for effect_id in player.team_effects:
                    ...
                    effect_desc = effect_info.get('description', '')
    ...
    def _game_main_loop(self):
        ...
        while not self.game_state.game_over:
            ...
            # 玩家行动 - 优化版：只在回合开始时询问是否继续出牌
            if not self.vs_ai or self.game_state.current_player == 0:
                self._player_turn_optimized(current_player)
            else:
    ...
    def _display_game_status(self):
        ...
        for i, player in enumerate(self.game_state.players):
            ...
            for char in player.characters:
                ...
                print(f"  {char.display_name}: {char.current_hp}/{char.max_hp} HP - {status}{defense_info}")
    ...
    def _player_turn_optimized(self, player: Player):
        """玩家回合 - 优化版本，简化流程"""
        print(f"\n{self._get_player_name(player.player_id - 1)} 的行动阶段")
        
        # 连续出牌，直到玩家选择结束回合或没有手牌
        while player.hand:
            print(f"\n当前手牌数：{len(player.hand)}")
            print("1. 使用卡牌")
            print("2. 结束回合")
            
            choice = input("请选择行动 (1-2): ").strip()
    ...
    def _display_brief_status(self):
        ...
        for i, player in enumerate(self.game_state.players):
            ...
            for char in player.characters:
                if char.is_alive:
                    chars_info.append(f"{char.display_name}({char.current_hp})")
                else:
                    chars_info.append(f"{char.display_name}(倒下)")
            print(f"{player_name}: {', '.join(chars_info)}")
    ...
    def _use_card_interactive(self, player: Player) -> bool:
        ...
        for i, card in enumerate(player.hand, 1):
            print(f"{i}. {card.display_name} - {card.description}")
        print("0. 返回")
        ...
        for i, char in enumerate(alive_chars, 1):
            print(f"{i}. {char.display_name} (HP: {char.current_hp}/{char.max_hp})")
        ...
        # 选择目标
        targets = self.engine.get_available_targets(player, selected_card)
        if not targets:
            print("没有可用的目标！")
            return False
        
        print(f"\n选择 {selected_card.display_name} 的目标:")
        for i, target in enumerate(targets, 1):
            ...
            print(f"{i}. {target.display_name} ({target_player_name}) (HP: {target.current_hp}/{target.max_hp})")
        ...
        if used_card:
            ...
            if success:
                print(f"\n{user_char.display_name} 使用了 {used_card.display_name}！")
                return True
            else:
                print("行动失败！")
            ...
            if used_card:
                ...
                if success:
                    ...
                    if target_player:
                        ...
                        target_player_name = "未知"
                    print(f"AI: {user_char.display_name} 对 {target_char.display_name} ({target_player_name}) 使用了 {used_card.display_name}！")
    ...
    def _display_game_result(self):
        ...
        for i, player in enumerate(self.game_state.players):
            ...
            for char in player.characters:
                status = "存活" if char.is_alive else "倒下"
                print(f"  {char.display_name}: {char.current_hp}/{char.max_hp} HP - {status}")
...
def main():
    ...
    try:
        ...
        if not characters or not action_cards:
            print("错误：游戏数据加载失败！")
            print("请确保 characters.yaml 和 action_cards.yaml 文件存在且格式正确。")
            return
        
        print(f"成功加载 {len(characters)} 个角色和 {len(action_cards)} 种行动卡")
