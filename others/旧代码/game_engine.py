...
class GameEngine:
    ...
    def _execute_attack(self, attacker_player: Player, attacker: Character, 
        ...
        if (attacker.character_type == CharacterType.CAFE and 
        ...
        # 俊+琉璃组合效果：每回合第一次攻击加1
        if (TeamEffect.JUN_LIULI in attacker_player.team_effects and 
            not attacker.first_damage_dealt):
            damage += 1
            attacker.first_damage_dealt = True
        ...
        if target_player and TeamEffect.YANGGUANG_LIULI in target_player.team_effects:
            # 检查是否是本回合第一次受到伤害
    ...
    def _check_jun_protection(self, original_target: Character, 
        ...
        for char in target_player.characters:
            if (char.character_type == CharacterType.JUN and 
                char.is_alive and 
    ...
    def process_turn_start(self, player: Player):
        ...
        yangguang_char = self._find_character_by_type(player, CharacterType.YANGGUANG)
    ...
    def process_turn_end(self, player: Player):
        ...
        xinhe_char = self._find_character_by_type(player, CharacterType.XINHE)
    ...
    def _find_character_by_type(self, player: Player, 
                               character_type: CharacterType) -> Optional[Character]:
        """在玩家队伍中查找指定类型的角色"""
        for char in player.characters:
    ...
    def get_available_targets(self, player: Player, action_type: ActionType) -> List[Character]:
        """获取行动卡的可用目标"""
        if action_type == ActionType.ATTACK:
            # 攻击卡只能攻击敌方存活角色
    ...
    def create_character(self, character_type: CharacterType) -> Character:
        """创建指定类型的角色"""
...
class GameAI:
    ...
    def make_decision(self, player: Player) -> List[Tuple[Character, ActionCard, Character]]:
        ...
        for card in player.hand[:]:  # 使用切片复制，避免修改时出错
            ...
            # 根据卡牌类型选择目标
            targets = self.engine.get_available_targets(player, card.action_type)
            if not targets:
                continue
                
            target_char = self._select_target(card.action_type, targets, alive_chars)
            if target_char:
                actions.append((user_char, card, target_char))
        
        return actions
    
    def _select_target(self, action_type: ActionType, available_targets: List[Character],
...
if __name__ == "__main__":
    ...
    # 创建测试角色
    char1 = engine.create_character(CharacterType.CAFE)
    char2 = engine.create_character(CharacterType.XINHE)
